...
import { ArtifactItem, KeyLinkItem } from './types.js';

/**
 * Internal artifact specification structure
 */
...
/**
 * Internal key link specification structure
 */
interface KeyLinkSpec {
  from?: string;
  to?: string;
  via?: string;
  pattern?: string;
...
export function cmdVerifyArtifacts(cwd: string, planFilePath: string, raw: boolean): void {
  ...
  for (const artifactRaw of artifacts) {
    ...
    const artifact = artifactRaw as ArtifactSpec;
...
export function cmdVerifyKeyLinks(cwd: string, planFilePath: string, raw: boolean): void {
  ...
  for (const linkRaw of keyLinks) {
    ...
    const link = linkRaw as KeyLinkSpec;
    const check = {
      from: link.from,
      to: link.to,
      via: link.via || '',
