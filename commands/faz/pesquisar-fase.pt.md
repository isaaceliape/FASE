...
description: Pesquisa como implementar uma fase (standalone - geralmente use /faz:planejar-fase ao invés disso)
argument-hint: "[fase]"
allowed-tools:
  - Read
  - Bash
...
**Nota:** Este é um comando de research standalone. Para a maioria dos workflows, use `/faz:planejar-fase` que integra research automaticamente.
...
### Próximos Passos
- Para planejamento: `/faz:planejar-fase {phase_number}`
...
Após research:
- **Para planejar:** `/faz:planejar-fase {phase_number}` (integra RESEARCH.md automaticamente)
- **Para revisar:** User examina RESEARCH.md
- **Para re-research:** Execute `/faz:pesquisar-fase {phase_number}` novamente
